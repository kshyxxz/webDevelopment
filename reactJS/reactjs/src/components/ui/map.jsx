...
let projectionPatchApplied = false;

function patchMapLibreProjectionCrash() {
	if (projectionPatchApplied) return;

	const mapPrototype = MapLibreGL?.Map?.prototype;
	if (!mapPrototype || typeof mapPrototype.migrateProjection !== "function") {
		return;
	}

	const cameraPrototype = Object.getPrototypeOf(mapPrototype);
	if (
		!cameraPrototype ||
		typeof cameraPrototype.migrateProjection !== "function"
	) {
		return;
	}

	mapPrototype.migrateProjection = function patchedMigrateProjection(
		transform,
		cameraHelper,
	) {
		cameraPrototype.migrateProjection.call(this, transform, cameraHelper);
		if (this.painter) {
			this.painter.transform = transform;
		}

		const projectionName = this.style?.projection?.name;
		if (projectionName) {
			this.fire("projectiontransition", {
				newProjection: projectionName,
			});
		}
	};

	if (typeof mapPrototype.setTerrain === "function") {
		const originalSetTerrain = mapPrototype.setTerrain;
		mapPrototype.setTerrain = function patchedSetTerrain(terrain) {
			if (!this.style || typeof this.style._checkLoaded !== "function") {
				return this;
			}
	...
	projectionPatchApplied = true;
}

function getSafeProjection(value) {
	if (typeof value === "string" && value.trim().length > 0) {
		return value.trim();
	}

	if (typeof value === "object" && value !== null) {
		const candidate = value.name ?? value.type;
...
const Map = forwardRef(function Map(
	...
	useEffect(() => {
		...
		const map = new MapLibreGL.Map({
			...
			...viewport,
			style: initialStyle,
			...(safeProjection ? { projection: safeProjection } : {}),
		});
		...
		const styleDataHandler = () => {
			...
			styleTimeoutRef.current = setTimeout(() => {
				...
				if (safeProjection) {
					try {
						map.setProjection(safeProjection);
					} catch (error) {
						console.warn("Map projection update failed", error);
					}
				}
			}, 100);
		};
